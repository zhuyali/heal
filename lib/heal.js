    ...
    rd.each(currentPath, function(f, s, next) {
      ...
      if (f.indexOf('template') >= 0) {
        if (path.extname(f) === '.js') {
          var mv2 = util.format('mv %s/lib/template.js %s/lib/%s.js', currentPath, currentPath, name);
          ...
          var mv1 = util.format('mv %s/bin/template %s/bin/%s', currentPath, currentPath, name);
